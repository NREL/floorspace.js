...
var updateAdjacencyIssues = function(value) {
  ...
  /*
    ...
    */
  const almost_equal = 1e-3;
  const thresh = (parseFloat(settings.findAdjacencyThreshold)**2)/1000;
...
const initDatGui = function () {
  ...
  var f2 = gui.addFolder('Adjacency Issues');
  ...
  f2.add(settings, 'findAdjacencyThreshold', 0.5, 2.5, 0.5).name('Threshold').onFinishChange(function (value) {
      if (settings.findAdjacencyIssues) {
          removeSelection();
          setLocalStorage('findAdjacencyThreshold', value);
          updateAdjacencyIssues();
