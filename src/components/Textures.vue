...
export default {
  ...
  methods: {
    ...
    checkTexturesList() {
      const missingTextures = _.difference(
          textures,
          _.map(this.patterns, "id")
        ),
  ...
  computed: {
    modSuffixes() {
      return _.filter(Object.keys(this.modifiers), _.identity);
    },
    patterns() {
      return _.reject(
        ...
        (p) =>
          this.modSuffixes.some((ms) => _.endsWith(p.getAttribute("id"), ms))
      );
    },
    cssStyles() {
    ...
    windowStyles() {
      return _.map(this.patterns, (p) => p.getAttribute("id")).map(
        (id) => `
