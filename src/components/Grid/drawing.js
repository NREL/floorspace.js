...
function boxAroundWindow({ xScale, yScale, edge, offset }) {
  const { dx, dy } = unitPerpVector(edge.start, edge.end);
  return [
    [xScale(edge.start.x) + offset * dx, yScale(edge.start.y) + offset * dy],
    [xScale(edge.start.x) - offset * dx, yScale(edge.start.y) - offset * dy],
    [xScale(edge.end.x) - offset * dx, yScale(edge.end.y) - offset * dy],
    [xScale(edge.end.x) + offset * dx, yScale(edge.end.y) + offset * dy],
    [xScale(edge.start.x) + offset * dx, yScale(edge.start.y) + offset * dy],
  ];
}
...
function drawWindowToWallRatio(xScale, yScale, el, datum) {
  ...
  const selection = d3.select(el).selectAll(".window-wall-ratio").data([datum]);
  ...
  wwr
    ...
    .attr(
      "d",
      line(boxAroundWindow({ edge: datum, xScale, yScale, offset: 8 }))
    )
    .attr("data-texture", datum.texture || "diagonal-stripes-4");
}

function drawDoor(xScale, yScale, el, datum) {
  const selection = d3.select(el).selectAll(".door").data([datum]);
...
function drawSingleWindow(xScale, yScale, el, datum) {
  ...
  const selection = d3.select(el).selectAll(".single-window").data([datum]);
...
function drawRepeatingWindows(xScale, yScale, el, datum) {
  ...
  const selection = d3.select(el).selectAll(".repeating-windows").data([datum]);
    ...
    selection
      ...
      .each(function (d) {
        if (d.windowOrDoor === "door") {
          ...
          throw new Error(
            `unexpected window_definition_mode: ${d.window_definition_mode}`
          );
        }
      });
  }
  ...
  function chart(selection) {
    ...
    measure
      ...
      .attr(
        "x",
        (d) =>
          (1 - labelPosition) * xScale(d.start.x) +
          labelPosition * xScale(d.end.x)
      )
      .attr(
        "y",
        (d) =>
          (1 - labelPosition) * yScale(d.start.y) +
    ...
    const arrowHead = "M -6,-2 V 2 L0,0 Z",
      rotate = (d) => {
        ...
        const angle = (edgeDirection(d) * 180) / Math.PI,
          adjust =
            d.start.x === d.end.x
              ? d.start.y > d.end.y
                ? 0
    ...
    measure
      ...
      .attr(
        "transform",
        (d) =>
          `translate(${xScale(d.start.x)}, ${yScale(
            d.start.y
          )}) scale(2) rotate(${rotate(d)})`
      )
      .attr("d", arrowHead);
    ...
    measure
      ...
      .attr(
        "transform",
        (d) =>
          `translate(${xScale(d.end.x)}, ${yScale(d.end.y)}) scale(2) rotate(${
            180 + rotate(d)
          })`
      )
      .attr("d", arrowHead);
    ...
    selection
      .merge(
        selection
          .enter()
          .append("g")
          .classed("window-guideline component-guideline", true)
      )
      .selectAll(".distance-measure")
      .data(
        // there must be a better way to do this...
        selection
          .merge(selection.enter())
          .data()
...
export function drawDaylightingControl() {
  ...
  function chart(selection) {
    ...
    const dcE = selection
      .enter()
      .append("g")
      .attr("class", "daylighting-control");
    ...
    dc.select("circle.bg")
      ...
      .attr("r", 10);
    dc.select("path.quadrants").attr("d", (d) => {
      const x = xScale(d.x),
        y = yScale(d.y),
        r = 10;
      return `M${x} ${y} L${x + r} ${y} A ${r} ${r} 0 0 1 ${x} ${
        y + r
...
export function drawDaylightingControlGuideline() {
  let xScale = _.identity,
    yScale = _.identity;
  const drawMeasure = distanceMeasure().lineOffset(0).labelPosition(0.5);
  function chart(selection) {
    drawMeasure.xScale(xScale).yScale(yScale);
    selection.exit().remove();
    const guideE = selection
      .enter()
      .append("g")
      .classed("daylighting-control-guideline component-guideline", true);

    const data = _.flatMap(selection.merge(selection.enter()).data(), (d) => {
      const dir = unitVector(d.nearestEdge.v1, d.nearestEdge.v2),
        v = { start: d.loc, end: d.nearestEdge.proj },
        dotProduct =
          (v.end.x - v.start.x) * dir.dx + (v.end.y - v.start.y) * dir.dy,
        parallel = {
          start: v.start,
          end: {
...
function bestResizeCursor(xOff, yOff, rotation) {
  const norm = Math.sqrt(xOff * xOff + yOff * yOff),
    x = xOff / norm,
    y = yOff / norm,
  ...
  return _.minBy(RESIZE_CURSORS, (c) => distanceBetweenPoints(c.vec, pt))
    .cursor;
}
const rotateCursor =
...
export function drawImage() {
  ...
  function chart(selection) {
    ...
    const scaleTransform = (d) => {
        const pxCenter = { x: xScale(d.x), y: yScale(d.y) },
    ...
    const rotationAngle = (d) => {
        const pxOrigin = { x: xScale(d.x), y: yScale(d.y) },
          startAngle = edgeDirection({
            start: pxOrigin,
            end: { x: startX, y: startY },
          }),
          currAngle = edgeDirection({
            start: pxOrigin,
            end: { x: currX, y: currY },
          });
        return (
          (180 * (currAngle - startAngle)) / Math.PI +
    ...
    selection.exit().remove();
    const imageGroupE = selection
      .enter()
      .append("g")
      .attr("class", "image-group");
    const moveableWrapperE = imageGroupE
      .append("g")
      .attr("class", "moveable-wrapper");
    ...
    ["tl", "tr", "bl", "br"].forEach((corner) =>
      controlsE.append("circle").attr("class", `corner ${corner}`)
    );
    ...
    imageGroup.attr(
      "transform",
      (d) => `translate(${xScale(d.x)}, ${yScale(d.y)}) rotate(${d.r})`
    );
    ...
    imageGroup
      .select(".moveable-wrapper")
      .attr("transform", "translate(0,0)")
      .style("cursor", (d) =>
        d.current ? "move" : d.clickable ? "pointer" : null
      )
    ...
    imageGroup.each(function (d) {
      ...
      if (!d.current) {
        ig.select(".moveable-wrapper, .rotation-handle, .corner").on(
          ".drag",
          null
        );
      } else {
...
export default function drawMethods({
  xScale,
  yScale,
  updateImage,
  selectImage,
