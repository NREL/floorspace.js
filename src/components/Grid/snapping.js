...
export function gridSnapTargets(gridSpacing, { x, y }) {
  return [
    {
      x: Math.floor(x / gridSpacing) * gridSpacing,
      y: Math.floor(y / gridSpacing) * gridSpacing,
    },
    {
      x: (Math.floor(x / gridSpacing) + 1) * gridSpacing,
      y: Math.floor(y / gridSpacing) * gridSpacing,
    },
    {
      x: Math.floor(x / gridSpacing) * gridSpacing,
      y: (Math.floor(y / gridSpacing) + 1) * gridSpacing,
    },
    {
      x: (Math.floor(x / gridSpacing) + 1) * gridSpacing,
      y: (Math.floor(y / gridSpacing) + 1) * gridSpacing,
...
export function expandWindowAlongEdge(
  edge,
  center,
  { width, spacing, window_definition_mode, texture }
) {
  if (
    window_definition_mode === "Window to Wall Ratio" ||
  ...
  const theta = edgeDirection({ start: edge.v1, end: edge.v2 }),
...
export function findClosestWindow(windows, cursor) {
  const withDistance = windows.map((w) => {
    ...
    const dist =
      w.window_definition_mode === "Single Window"
...
function snapWindowToEdgeAtGridIntervals(
  edges,
  cursor,
  windowDefn,
  maxSnapDist,
  ...
  const dist = distanceBetweenPoints(closestEdge.v1, closestEdge.proj),
