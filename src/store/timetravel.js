...
export default {
  ...
  maybeSaveCheckpoint(action) {
    ...
    this.resetBatchFlag = setTimeout(() => {
      this.currentlyInBatch = false;
    }, 0);
  },
  /*
  ...
  potentiallyRollbackCheckpoint(action) {
    ...
    _.defer(() => {
      ...
      while (
        prevStates.length > 0 &&
        _.isEqual(
          serializeState(store.state),
          prevStates[prevStates.length - 1].state
        )
      ) {
        console.log(
          "current state matches previous, so rolling back",
          prevStates[prevStates.length - 1].triggeringAction
        );
        prevStates.pop();
      }
    });
  ...
  undo() {
    ...
    const { state: replacementState, triggeringAction } =
      this.pastTimetravelStates.pop();
  ...
  redo() {
    ...
    const { state: replacementState, triggeringAction } =
      this.futureTimetravelStates.pop();
    const newState = JSON.parse(replacementState);
    this.pastTimetravelStates.push({
      state: serializeState(this.store.state),
      triggeringAction: this.triggeringAction,
  ...
  logTimetravel() {
    console.log(
      "past:",
      this.pastTimetravelStates.map((s) => logState(s))
    );
    console.log("current", logState(this.store.state));
    console.log(
      "future:",
      this.futureTimetravelStates.map((s) => logState(s))
    );
  },
};
