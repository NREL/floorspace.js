...
function componentsOnStory(state, geometry_id) {
  const geom = geometryHelpers.denormalize(
      state.geometry.find((g) => g.id === geometry_id)
    ),
    story = state.models.stories.find((s) => s.geometry_id === geometry_id);

  const perFaceComponents = story.spaces.map((space) => ({
    daylighting_controls: space.daylighting_controls.map((d) => ({
      ...d,
      vertex: _.find(geom.vertices, { id: d.vertex_id }),
    })),
    space_id: space.id,
  ...
  return {
    geometry_id,
    story_id: story.id,
    windows: story.windows.map((w) => {
      const edge = _.find(geom.edges, { id: w.edge_id }),
    ...
    doors: story.doors.map((d) => {
      const edge = _.find(geom.edges, { id: d.edge_id }),
...
function replaceComponents(context, components, movementsByFaceId = {}) {
  const { geometry_id, story_id, windows, doors, perFaceComponents } =
      components,
    geometry = geometryHelpers.denormalize(
      _.find(context.rootState.geometry, { id: geometry_id })
    ),
    story = _.find(context.rootState.models.stories, { id: story_id }),
    spaceFaces = geometry.faces.filter((f) =>
      _.find(story.spaces, { face_id: f.id })
    ),
  ...
  const replaceWindowOrDoor = (arr, windowOrDoor) => {
    ...
    arr.forEach((w) => {
      const { dx, dy } = movementsByFaceId[w.originalFaceIds[0]] || {
          dx: 0,
          dy: 0,
        },
      ...
      const facesWithEdge = facesContainingEdge(spaceFaces, loc.edge_id).map(
        (f) => f.id
      );
      if (
        w.originalFaceIds.length !== facesWithEdge.length ||
        (w.originalFaceIds.length > 1 &&
          _.intersection(Object.keys(movementsByFaceId), w.originalFaceIds))
      ) {
      ...
      windowsOrDoors.push({
        ...
        [`${windowOrDoor.toLowerCase()}_definition_id`]:
          w.window_definition_id || w.door_definition_id,
      });
    });

    context.dispatch(`models/create${windowOrDoor}s`, windowsOrDoors, {
      root: true,
    });
  };
  ...
  perFaceComponents.forEach(({ face_id, daylighting_controls }) => {
    daylighting_controls.forEach((d) => {
      const { dx, dy } = movementsByFaceId[face_id] || { dx: 0, dy: 0 },
      ...
      context.dispatch(
        "models/createDaylightingControl",
        {
          daylighting_control_definition_id:
            d.daylighting_control_definition_id,
          x: loc.x,
          y: loc.y,
          story_id,
          face_id: loc.face_id,
...
export function withPreservedComponents(...args) {
  ...
  if (args.length === 3) {
    ...
    throw new Error(
      "usage: withPreservedComponents(context, geometry_id, action [,movementsByFaceId])"
    );
  }
