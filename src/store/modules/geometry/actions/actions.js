...
export default {
  ...
  moveFaceByOffset(context, payload) {
    const { face_id, dx, dy } = payload,
      ...
      movedPoints = geometryHelpers
        .verticesForFaceId(face.id, currentStoryGeometry)
        .map((v) => ({
          x: v.x + dx,
          y: v.y + dy,
        })),
  ...
  destroyFaceAndDescendents(context, payload) {
    ...
    const geometry = context.state.find((g) => g.id === geometry_id),
      ...
      expVertices = face.edgeRefs
        ...
        .filter(
          (v) => geometryHelpers.facesForVertexId(v.id, geometry).length < 2
        ),
      expEdgeRefs = face.edgeRefs.filter(
        (edgeRef) =>
          geometryHelpers.facesForEdgeId(edgeRef.edge_id, geometry).length < 2
      );

    // destroy face, then edges, then vertices
    context.commit("destroyGeometry", { id: face.id });
    expEdgeRefs.forEach((edgeRef) =>
      context.commit("destroyGeometry", { id: edgeRef.edge_id })
    );
    expVertices.forEach((vertex) =>
      context.commit("destroyGeometry", { id: vertex.id })
    );
  },
  ...
  replaceFacePoints(context, { geometry_id, face_id, vertices, edges }) {
    ...
    const replacementVertIds = _.chain(vertices)
      .map((vert) => {
        const gVert = _.find(
          geom.vertices,
          (v) => distanceBetweenPoints(v, vert) < spacing / 20
        );
    ...
    updatedEdges.forEach((edge) => {
      const gEdge =
        _.find(geom.edges, { v1: edge.v1, v2: edge.v2 }) ||
        _.find(geom.edges, { v1: edge.v2, v2: edge.v1 });
  ...
  trimGeometry({ rootState, commit }, { geometry_id }) {
    const story = _.find(rootState.models.stories, { geometry_id }),
      vertsReferencedByDCs = _.flatMap(story.spaces, (s) =>
        _.map(s.daylighting_controls, "vertex_id")
      );

    commit("trimGeometry", {
      geometry_id,
      vertsReferencedElsewhere: vertsReferencedByDCs,
    });
  },
  cloneStoryGeometry(context, payload) {
    const currentStory =
      context.rootGetters["application/currentStoryGeometry"];
    context.commit("updateClonedGeometry", {
      newGeom: payload,
