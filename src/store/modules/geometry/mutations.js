...
export function trimGeometry(state, { geometry_id, vertsReferencedElsewhere }) {
  const geometry = _.find(state, { id: geometry_id }),
    edgesInUse = new Set(
      _.flatMap(geometry.faces, (f) => _.map(f.edgeRefs, "edge_id"))
    ),
  ...
  if (missingEdges.length) {
    console.error(
      "An edge is referenced by a face, but does not exist!",
  ...
  if (missingVerts.length) {
    console.error(
      "A vertex is referenced by a face, but does not exist!",
      JSON.stringify(geometry)
    );
  }
...
export function destroyGeometry(state, payload) {
  ...
  for (let i = 0; i < state.length; i++) {
    ...
    if (g.id === id) {
      state.splice(
        state.findIndex((g) => g.id === id),
        1
      );
      break;
    } else if (~g.faces.map((f) => f.id).indexOf(id)) {
      g.faces.splice(
        g.faces.findIndex((f) => f.id === id),
        1
      );
      break;
    } else if (~g.edges.map((e) => e.id).indexOf(id)) {
      g.edges.splice(
        g.edges.findIndex((e) => e.id === id),
        1
      );
        ...
        face.edgeRefs.forEach((edgeRef) => {
          if (id === edgeRef.edge_id) {
            face.edgeRefs.splice(
              face.edgeRefs.findIndex((r) => r.edge_id === id),
              1
            );
          }
        });
      ...
      g.vertices.splice(
        g.vertices.findIndex((v) => v.id === id),
        1
      );
      break;
    }
...
export function setEdgeRefsForFace(state, payload) {
  const { geometry_id, face_id, edgeRefs } = payload,
    geometry = state.find((g) => g.id === geometry_id),
    face = geometry.faces.find((f) => f.id === face_id);
  face.edgeRefs = edgeRefs.filter((ref) =>
    geometry.edges.find((e) => e.id === ref.edge_id)
  );
}
...
export function destroyEdgeRef(state, payload) {
  const { geometry_id, face_id, edge_id } = payload,
    ...
    face = geometry.faces.find((f) => f.id === face_id);
  face.edgeRefs.splice(
    face.edgeRefs.findIndex((eR) => eR.edge_id === edge_id),
    1
  );
}

export function replaceEdgeRef(state, payload) {
  const { geometry_id, face_id, edge_id, newEdges } = payload,
    geometry = state.find((g) => g.id === geometry_id),
    face = geometry.faces.find((f) => f.id === face_id),
...
export function ensureVertsExist(state, { geometry_id, vertices }) {
  const geometry = _.find(state, { id: geometry_id });
  vertices.forEach(
    (v) => _.find(geometry.vertices, { id: v.id }) || geometry.vertices.push(v)
  );
}
...
export function ensureEdgesExist(state, { geometry_id, edges }) {
  const geometry = _.find(state, { id: geometry_id });
  edges.forEach(
    (e) => _.find(geometry.edges, { id: e.id }) || geometry.edges.push(e)
  );
}

export function splitEdge(
  state,
  { geometry_id, edgeToDelete, newEdges, replaceEdgeRefs }
) {
...
export function replaceFacePoints(
  state,
  { geometry_id, vertices, edges, face_id }
) {
  const geometry = _.find(state, { id: geometry_id });
...
export function updateClonedGeometry(state, payload) {
  const newGeometry = state.find(
    (story) => story.id === payload.currentStoryId
  );
