...
export default {
  ...
  setUnits({ commit, dispatch, rootState }, payload) {
    if (payload.units === "m" || payload.units === "ft") {
      ...
      rootState.models.stories.forEach((story) =>
        dispatch("models/destroyStory", { story }, { root: true })
      );
    }
  },
  ...
  setNorthAxis(context, payload) {
    ...
    context.commit("setMapRotation", {
      rotation: (north_axis * Math.PI) / 180,
    });
  },
  ...
  setPreview3DEnabled(context, payload) {
    context.commit("setPreview3DEnabled", payload);
  },
  ...
  setMapRotation(context, payload) {
    context.commit("setConfigNorthAxis", {
      north_axis: (payload.rotation / (2 * Math.PI)) * 360,
    });
