...
export default {
  ...
  oneOf(...args) {
    return (object, store, value, type) => {
      if (!_.includes(args, value)) {
        return {
          success: false,
          error: `Expected one of ${args}, got ${value}`,
        };
      }
      return { success: true };
    };
  ...
  gt0(object, story, value, type) {
    return value > 0
      ? { success: true }
      : { success: false, error: "Expected value to be greater than zero" };
  },
  ...
  halfOpenInterval(start, end) {
    return (object, store, value, type) => {
      if (value < start || value >= end) {
        return {
          success: false,
          error: `Must be greater than or equal ${start} and less than ${end}`,
        };
      }
      return { success: true };
    };
  ...
  gt0orNull(object, story, value, type) {
    ...
    return {
      success: false,
      error: "If provided, value must be greater than 0",
    };
  },
};
