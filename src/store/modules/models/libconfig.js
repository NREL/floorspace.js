  ...
  window_definitions: {
    ...
    columns: [
      ...
      },
      {
        ...
        enabled(row) {
          return (
            row.window_definition_mode === "Single Window" ||
    ...
    columns: [
      ...
      },
      {
        ...
        select_data(space, state) {
          ...
          state.models.library.building_units.forEach((b) => {
            options[b.name] = b.id;
          });
          return options;
        },
        get(space, state) {
          const buildingUnit = state.models.library.building_units.find(
            (b) => b.id === space.building_unit_id
          );
      ...
      },
      {
        ...
        select_data(_space, state) {
          const options = { "(none)": null };
          state.models.library.building_types.forEach((b) => {
            options[b.name] = b.id;
          });
          return options;
        },
        get(space, state) {
          const buildingType = state.models.library.building_types.find(
            (b) => b.id === space.building_type_id
          );
      ...
      },
      {
        ...
        select_data(space, state) {
          const options = { "(none)": null };
          state.models.library.space_types.forEach((s) => {
            options[s.name] = s.id;
          });
          return {
            ...options,
            ...helpers.spaceTypesForBuilding(
              space.building_type_id,
              space.template
            ),
          };
        },
        get(space, state) {
          const spaceType = state.models.library.space_types.find(
            (s) => s.id === space.space_type_id
          );
      ...
      },
      {
        ...
        select_data(_space, state) {
          const options = { "(none)": null };
          state.models.library.thermal_zones.forEach((t) => {
            options[t.name] = t.id;
          });
          return options;
        },
        get(space, state) {
          const thermalZone = state.models.library.thermal_zones.find(
            (b) => b.id === space.thermal_zone_id
          );
      ...
      },
      {
        ...
        select_data(space, state) {
          const options = { "(none)": null };
          state.models.library.construction_sets.forEach((c) => {
            options[c.name] = c.id;
          });
          return options;
        },
        get(space, state) {
          const constructionSet = state.models.library.construction_sets.find(
            (c) => c.id === space.construction_set_id
          );
      ...
      },
      {
        ...
        select_data(space, state) {
          return {
            "(none)": null,
            ..._.fromPairs(
              state.models.library.pitched_roofs.map((prt) => [
                prt.name,
                prt.id,
              ])
            ),
        ...
        get(space, state) {
          const prt = _.find(state.models.library.pitched_roofs, {
            id: space.pitched_roof_id,
          });
  ...
  windows: {
    ...
    columns: [
      ...
      },
      {
        ...
        select_data(space, state) {
          return _.fromPairs(
            state.models.library.window_definitions.map((b) => [b.name, b.id])
          );
        },
      },
    ],
  ...
  daylighting_controls: {
    ...
    columns: [
      ...
      },
      {
        ...
        select_data(space, state) {
          return _.fromPairs(
            state.models.library.daylighting_control_definitions.map((b) => [
              b.name,
              b.id,
            ])
          );
  ...
  doors: {
    ...
    columns: [
      ...
      },
      {
        ...
        select_data(space, state) {
          return _.fromPairs(
            state.models.library.door_definitions.map((b) => [b.name, b.id])
          );
        },
      },
    ],
...
Object.keys(map).forEach((k) => {
  map[k].keymap = _.zipObject(_.map(map[k].columns, "name"), map[k].columns);
