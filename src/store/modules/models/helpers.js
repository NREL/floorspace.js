...
const helpers = {
  ...
  displayNameForKey(type, key) {
    if (this.map[type].keymap[key]) {
      return this.map[type].keymap[key].private
        ? null
        : this.map[type].keymap[key].displayName;
    }
    return key;
  },
  ...
  setValueForKey(object, store, type, key, value) {
    ...
    if (this.map[type].keymap[key] && this.map[type].keymap[key].converter) {
      value = this.map[type].keymap[key].converter(
        value,
        this.map[type].keymap[key]
      );
    }
    ...
    if (this.map[type].keymap[key] && this.map[type].keymap[key].validator) {
      const validationResult = this.map[type].keymap[key].validator(
        object,
        store,
        value,
        type
  ...
  inputTypeForKey(type, key) {
    return this.map[type].keymap[key] && !this.map[type].keymap[key].readonly
      ? this.map[type].keymap[key].input_type
      : null;
  },

  selectOptionsForKey(object, state, type, key) {
    return this.map[type].keymap[key] &&
      !this.map[type].keymap[key].readonly &&
      this.map[type].keymap[key].input_type === "select"
      ? this.map[type].keymap[key].select_data(object, state)
      : [];
  },
  ...
  libraryObjectWithId(state, id) {
    ...
    for (var type in state.library) {
      if (
        state.library.hasOwnProperty(type) &&
        !result &&
        state.library[type].find((i) => i.id === id)
      ) {
    ...
    if (building_type == "SecondarySchool") {
      ...
      if (
        ...
        if (wholeBuilding) {
          ...
          hash["Office - Large Main Data Center"] =
  ...
  replaceIdsUpdateInfoForCloning(story, idMap) {
    ...
    clonedStory.spaces = clonedStory.spaces.map((space) => {
      ...
      space.daylighting_controls = space.daylighting_controls.map(
        (daylighting_control) => {
          const newID = idFactory.generate();
