...
export default {
  ...
  destroyStory(context, payload) {
    ...
    context.dispatch("geometry/destroyGeometry", story.geometry_id, {
      root: true,
    });
    if (stories.length === 0) {
  ...
  destroySpace(context, payload) {
    ...
    if (face) {
      ...
      context.dispatch(
        "geometry/destroyFaceAndDescendents",
        {
          face: face,
          geometry_id: geom.id,
        },
  ...
  destroyShading(context, payload) {
    ...
    const face = context.rootGetters[
      "application/currentStoryGeometry"
    ].faces.find((f) => f.id === shading.face_id);
    if (face) {
      // destroy face associated with the space
      context.dispatch(
        "geometry/destroyFaceAndDescendents",
        {
          face,
          geometry_id:
            context.rootGetters["application/currentStoryGeometry"].id,
        },
        { root: true }
      );
    }
  ...
  updateSpaceWithData(context, payload) {
    const space = context.getters.allSpaces.find(
        (s) => s.id === payload.space.id
      ),
  ...
  updateShadingWithData(context, payload) {
    const shading = context.getters.allShading.find(
        (s) => s.id === payload.shading.id
      ),
  ...
  updateImageWithData(context, payload) {
    const image = context.getters.allImages.find(
        (i) => i.id === payload.image.id
      ),
      validProperties = _.difference(Object.keys(image), [
        "src",
        "naturalWidth",
        "naturalHeight",
      ]);
  ...
  updateWindowDefinitionWithData({ state, dispatch }, payload) {
    ...
    const windowDefnDefaults = _.pick(getDefaults("WindowDefinition"), [
      "height",
      "width",
      "window_spacing",
      "wwr",
    ...
    if (payload.window_definition_mode === "Window to Wall Ratio") {
      ...
      throw new Error(
        `unrecognized window_definition_mode: ${payload.window_definition_mode}`
      );
    }

    if (
      _.includes(
        ["Window to Wall Ratio", "Repeating Windows"],
        payload.window_definition_mode
      )
    ) {
  ...
  updateObjectWithData(context, payload) {
    const object = helpers.libraryObjectWithId(
      context.state,
      payload.object.id
    );
  ...
  createObjectWithTypeAndSelect(context, payload) {
    ...
    context.commit("initObjectWithSelection", {
      space_id,
      type,
      type_id,
      object,
  ...
  createWindow(context, payload) {
    const { story_id, edge_id, window_definition_id, alpha } = payload,
      story = _.find(context.state.stories, { id: story_id }),
      windowDefn = _.find(context.state.library.window_definitions, {
        id: window_definition_id,
      }),
      geometry =
        story && _.find(context.rootState.geometry, { id: story.geometry_id }),
    ...
    const windowsOnEdge = _.filter(story.windows, { edge_id }).map((w) => ({
      ...w,
      window_definition_mode: _.find(context.state.library.window_definitions, {
    ...
    if (windowDefn.window_definition_mode === "Single Window") {
      ...
      windowsToDelete = _.reject(windowsOnEdge, {
    ...
    windowsToDelete.forEach((w) =>
      context.commit("destroyWindow", { story_id, object: { id: w.id } })
    );
  ...
  createWindows(context, payload) {
    ...
    payload.forEach((p) => {
      const { story_id, edge_id, window_definition_id, alpha } = p,
        story = _.find(context.state.stories, { id: story_id }),
        windowDefn = _.find(context.state.library.window_definitions, {
          id: window_definition_id,
        }),
        geometry =
          story &&
          _.find(context.rootState.geometry, { id: story.geometry_id }),
        edge = geometry && _.find(geometry.edges, { id: edge_id });
      if (!story) {
        throw new Error("Story not found");
      } else if (!windowDefn) {
      ...
      windowsToDelete.forEach((w) =>
        deletes.push({ story_id, object: { id: w.id } })
      );
  ...
  createDoor(context, payload) {
    const { story_id, edge_id, door_definition_id, alpha } = payload,
      story = _.find(context.state.stories, { id: story_id }),
      doorDefn = _.find(context.state.library.door_definitions, {
  ...
  createDaylightingControl(context, payload) {
    const { story_id, face_id, daylighting_control_definition_id, x, y } =
        payload,
      story = _.find(context.state.stories, { id: story_id }),
      daylightingDefn = _.find(
        context.state.library.daylighting_control_definitions,
        { id: daylighting_control_definition_id }
      ),
      geometry =
        story && _.find(context.rootState.geometry, { id: story.geometry_id }),
      ...
      vertex =
        geometry &&
        (geometryHelpers.vertexForCoordinates({ x, y }, geometry) ||
          geometryFactory.Vertex(x, y));
    if (!story) {
      throw new Error("Story not found");
    ...
    context.commit(
      "geometry/ensureVertsExist",
      { geometry_id: geometry.id, vertices: [vertex] },
      { root: true }
    );
