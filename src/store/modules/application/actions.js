...
export default {
  ...
  setCurrentComponentId(context, payload) {
    ...
    if (
      ...
      (context.getters.currentStory.windows.find((i) => i.id === id) ||
        context.getters.currentStory.daylighting_controls.find(
          (i) => i.id === id
        ))
    ) {
  ...
  setCurrentComponentDefinitionId(context, payload) {
    const { id } = payload;
    if (
      context.rootState.models.library.daylighting_control_definitions.find(
        (i) => i.id === id
      ) ||
      context.rootState.models.library.window_definitions.find(
        (i) => i.id === id
      ) ||
      ...
      throw new Error(
        `unable to set component definition -- couldn't find one with id ${id}`
      );
    }
  },
  ...
  setCurrentComponentInstanceId(context, payload) {
    ...
    if (!componentInstanceById(context.getters.currentStory, id)) {
      console.error(
        "unable to find an instance with that id on the current story"
      );
    } else {
