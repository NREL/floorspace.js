...
export default {
  ...
  currentStory(state, getters, rootState) {
    console.log("trying to get currentStory", rootState.models.stories);
    return rootState.models.stories.find(
      (s) => s.id === state.currentSelections.story_id
    );
  },
  ...
  currentSubSelection(state, getters) {
    ...
    const subSelectionId =
      state.currentSelections.subselection_ids[currentStory.id];
    return (
      currentStory.spaces.find((i) => i.id === subSelectionId) ||
  ...
  currentSpace(state, getters) {
    return getters.currentSubSelectionType === "spaces"
      ? getters.currentSubSelection
      : null;
  },
  currentShading(state, getters) {
    return getters.currentSubSelectionType === "shading"
      ? getters.currentSubSelection
      : null;
  },
  currentImage(state, getters) {
    return getters.currentSubSelectionType === "images"
      ? getters.currentSubSelection
      : null;
  },
  ...
  currentComponent(state, getters, rootState) {
    const componentDefinitionId =
      state.currentSelections.component_definition_id;
    let type, definition;
    if (
      (definition = rootState.models.library.window_definitions.find(
        (i) => i.id === componentDefinitionId
      ))
    ) {
      type = "window_definitions";
    } else if (
      (definition =
        rootState.models.library.daylighting_control_definitions.find(
          (i) => i.id === componentDefinitionId
        ))
    ) {
      type = "daylighting_control_definitions";
    } else if (
      (definition = _.find(rootState.models.library.door_definitions, {
  ...
  currentSpaceProperty(state, getters, rootState) {
    const spacePropertyId = state.currentSelections.space_property_id,
  ...
  currentSubSelectionFace(state, getters) {
    ...
    if (
      getters["currentSubSelection"] &&
      getters["currentSubSelection"].face_id
    ) {
      return getters["currentStoryGeometry"].faces.find(
        (f) => f.id === getters["currentSubSelection"].face_id
      );
    }
    return null;
  },
