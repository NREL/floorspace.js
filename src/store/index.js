  ...
  actions: {
    ...
    changeUnits(context, { newUnits }) {
      ...
      if (oldUnits !== "ip" && oldUnits !== "si") {
        console.error(
          `Expected data.project.config.units to be "ip" or "si", received "${oldUnits}"`
        );
      }
      if (newUnits !== "ip" && newUnits !== "si") {
        console.error(
          `Expected oldUnits to be "ip" or "si", received "${newUnits}"`
        );
      }
      context.commit(
        "changeUnits",
        convertState(context.state, oldUnits, newUnits)
      );
